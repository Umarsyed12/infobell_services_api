...
@app.route('/dataFocus', methods=['GET','POST'])
def fetchData():
    try:
        dict = {}
        ...
        for i in range(len(focus)):
            if focus[i]=="cpu":
                ...
                cpu = responce(column_name, data)
                print(len(cpu))
                dict.update({"CPU Utilization": cpu})
            elif focus[i]=="memory":
                create_anomaly_Memory(Host, start_date, end_date)
                data, column_name = create_anomaly_data_memory(percentile, Host, Disk,nics)
                memory = responce(column_name, data)
                print(len(memory))
                dict.update({"Memory Utilization": memory})
            elif focus[i]=="disk":
                ...
                disk = responce(column_name, data)
                print(len(disk))
                dict.update({"Disk Utilization": disk})
            elif focus[i]=="diskW":
                ...
                diskW = responce(column_name, data)
                print(len(diskW))
                dict.update({"DiskWeigted Utilization": disk})
        resp_data = {"Data": dict}
