...
def create_anomaly_cpu(host, start_date, end_date):  # create temp table and calculate rank and percentile
...
def create_anomaly_data_cpu(percentile, Host, Disk, nics):  # Get the data from selected percentile
    anomaly_query = ('''
    Select A.*, 
...
def create_anomaly_Memory(host, start_date, end_date):  # create temp table and calculate rank and percentile
...
def create_anomaly_data_memory(percentile, Host, Disk, nics):  # Get the data from selected percentile
    anomaly_query = ('''
    Select A.*, 
...
def create_anomaly_DiskBusy(host, disk, start_date, end_date):  # create temp table and calculate rank and percentile
...
def create_anomaly_data_DiskBusy(percentile, Host, Disk, nics):  # Get the data from selected percentile
    mycursor = base.cursor()
    anomaly_query = ('''
    Select A.*,
    B."utilization" as "cpu_utilization",
...
def create_anomaly_DiskWeighted(host, disk, start_date,
                                end_date):  # create temp table and calculate rank and percentile
...
def create_anomaly_data_DiskWeighted(percentile, Host, Disk, nics):  # Get the data from selected percentile
    anomaly_query = ('''
    Select A.*,
    B."utilization" as "cpu_utilization",
    C."utilization" as "memory_utilization",
...
def output_response(data, column_name, focus):  # to get output
    try:
        resp_dict = {}
...
@app.route('/percentile', methods=['POST'])   #Get the data of two percentile
