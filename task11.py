...
def create_anomaly_cpu(host, start_date, end_date):
    anomaly_query = (''' DROP TABLE If Exists "cpu_percentiles";
        ...
        Set "percentile" = Round(Round(Cast(Rank AS Decimal) /(Select count(*)
         from "cpu_percentiles"), 5)*100,3);
        ''')
...
def output_response(data, column_name):
    try:
        ...
        for index in range(0, index_len): #(0,11)x
...
@app.route('/getAnomalyByData',methods=["POST"])
def api():
    try:
        ...
        resp_dict = output_response(data, column_name)

        resp_data = {"Output": resp_dict}
