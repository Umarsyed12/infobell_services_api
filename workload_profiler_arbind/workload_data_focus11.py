...
def create_anomaly_Memory(host, start_date, end_date): #create temp table and calculate percentile
...
def create_anomaly_data_memory(percentile, Host, Disk,nics): #getting data and column_name
    anomaly_query = ('''
    Select A.*, 
...
def create_anomaly_DiskBusy(host, disk, start_date, end_date): #create temp table and calculate percentile
...
def create_anomaly_data_DiskBusy(percentile, Host, Disk,nics): #getting data and column_name
    mycursor = base.cursor()
    anomaly_query = ('''
    Select A.*,
    B."utilization" as "cpu_utilization",
...
def create_anomaly_DiskWeighted(host, disk, start_date, end_date):  #create temp table and calculate percentile
...
def create_anomaly_data_DiskWeighted(percentile, Host, Disk,nics): #getting data and column_name
    anomaly_query = ('''
    Select A.*,
    B."utilization" as "cpu_utilization",
    C."utilization" as "memory_utilization",
...
def fetchData():  # query functions of workload comparision for selected data focus and selected workload, get the response in json format(date range
    try:
